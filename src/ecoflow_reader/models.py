...
class BatteryStatus(EcoFlowBaseModel):
    ...
    target_level: float = Field(
        default=0.0, alias="bms_bmsStatus.targetSoc", description="Nivel de carga objetivo en %"
    )
...
class PowerIn(EcoFlowBaseModel):
    ...
    time_remaining_mins: int = Field(
        default=0, alias="bms_emsStatus.chgRemainTime", description="Minutos para carga completa"
    )
...
class PowerOut(EcoFlowBaseModel):
    ...
    time_remaining_mins: int = Field(
        default=0, alias="pd.remainTime", description="Minutos de batería restante (negativo)"
    )

    # Interruptores (Switches)
    ac_enabled: int = Field(
        default=0, alias="inv.cfgAcEnabled", description="Estado de los enchufes AC (1=Encendido, 0=Apagado)"
    )
    dc_enabled: int = Field(
        default=0, alias="pd.dcOutState", description="Estado de los puertos DC/USB (1=Encendido, 0=Apagado)"
    )
